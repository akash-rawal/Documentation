...
<Info>
Only users with **workspace admin** privileges can enable IP Allowlisting, add IP ranges, and remove them.
</Info>
...
Once IP Allowlisting is enabled, only users connecting from the listed IP ranges can work in the tool — accessing data such as videos or images and performing annotations. Users connecting from unlisted IPs are blocked, including the **workspace admin** — after enabling the allowlist, even the workspace admin can only access the workspace from the listed IPs. Only a **workspace admin** can enable the allowlist, add IP ranges, and remove them.
...
Only a **workspace admin** can manage the allowlist from the same **Workspace Settings** screen:
...
To disable the allowlist, the workspace admin must first connect from a listed IP — the allowlist applies to the workspace admin as well.
